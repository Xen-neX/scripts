...
echo "Версия 2.1 - Полное прозрачное проксирование трафика с использованием shadowsocks-rust"
...
# Определяем путь к sslocal
SSLOCAL_PATH="/tmp/shadowsocks-*/sslocal"
SSLOCAL_DEST="/usr/local/bin/sslocal"

echo "Перемещаю бинарный файл sslocal в /usr/local/bin/..."
mv $SSLOCAL_PATH $SSLOCAL_DEST

# Проверяем, успешно ли перемещён sslocal
if [ ! -f "$SSLOCAL_DEST" ]; then
    echo "Ошибка: sslocal не найден в /usr/local/bin/. Проверьте установку."
    exit 1
fi

chmod +x $SSLOCAL_DEST
